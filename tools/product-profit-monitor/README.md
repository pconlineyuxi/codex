...
下方“就这份数据继续提问”使用 `.env` 中的 `OPENAI_MODEL` / `OPENAI_API_KEY`。仅在用户提问时调用模型，发送计算结果、选定范围的汇总数据以及最近六轮对话，不发送订单行。全范围利润和费用贡献始终提供；汇总记录最多发送 160 条，优先问题中提及的对象，其余按利润绝对值排列，覆盖不全会明确标记。模型没有查询/写入数据库或发送通知的工具，回答可以展开核对其证据编号与数据。
